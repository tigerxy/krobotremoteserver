        ...
        myXmlRpc(path) { request ->
            try {
                when (request.methodName) {
                    ...
                    ROBOT_KEYWORD_TAGS -> MethodResponseSuccess(lib.getKeywordTags(request.methodName))
                    ROBOT_KEYWORD_DOCUMENTATION -> MethodResponseSuccess(lib.getKeywordDocumentation(request.methodName))
                    ROBOT_KEYWORD_ARGUMENTS -> MethodResponseSuccess(lib.getKeywordArguments(request.methodName))
                    ROBOT_KEYWORD_TYPES -> MethodResponseSuccess(lib.getKeywordTypes(request.methodName))
                    ...
                    else -> MethodResponseFault(1, "unknown method ${request.methodName}")
                }
            } catch (e: Exception) {
