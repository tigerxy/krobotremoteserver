...
internal class Deserializer internal constructor() {
    ...
    internal fun parseMethodCall(methodCallElement: Element): MethodCall {
        ...
        return MethodCall(
            ...
            children["params"]?.mapChildren { paramElement ->
                ...
                parseValue(
                    paramElement.associateChildren()["value"] ?: error("<param> missing a <value>")
                )
            } ?: emptyList()
        )
    }
    ...
    internal fun parseMethodResponse(methodResponseElement: Element): MethodResponse {
        ...
        return when (tagName) {
            "fault" -> {
                ...
                MethodResponseFault(
                    (faultStruct.members["faultCode"] as? IntegerValue)?.value
                        ?: error("missing faultCode in <fault>"),
                    (faultStruct.members["faultString"] as? StringValue)?.value
                        ?: error("missing faultString in <fault>")
                )
            }
            ...
            "params" -> {
                MethodResponseSuccess(
                    element.mapChildren { paramElement ->
                        if (paramElement.tagName != "param") error("expecting <param>, got <${paramElement.tagName}>")
                        parseValue(
                            paramElement.associateChildren()["value"]
                                ?: error("<param> missing a <value>")
                        )
                    }
                )
            }
    ...
    internal fun parseValue(valueElement: Element): Value {
        ...
        return when (tagName) {
            ...
            "array" -> {
                val dataElement = element.associateChildren()["data"]
                    ?: error("missing <data> element in <array>")
