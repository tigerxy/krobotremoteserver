...
internal class Serializer internal constructor(private val document: Document) {
    ...
    internal fun Value.appendToElement(parent: Node) {
        ...
        when (this) {
            ...
            is BooleanValue -> valueElement.appendNewElementWithTextContent(
                "boolean",
                if (value) "1" else "0"
            )

            is DateTimeIso8601Value -> valueElement.appendNewElementWithTextContent(
                "datetime.iso8601",
                value
            )

            is DoubleValue -> valueElement.appendNewElementWithTextContent(
                "double",
                value.toString()
            )
    ...
    private fun Node.appendNewElementWithTextContent(tagName: String, text: String) =
        appendChild(createElementWithTextContent(tagName, text))
}
